...
export function HappinessPage({
  ...
  const inferEmoji = (text: string): string => {
    ...
    if (lowerText.includes('thank') || lowerText.includes('grateful')) return '🙏';
    if (lowerText.includes('love') || lowerText.includes('heart')) return '💛';
    if (lowerText.includes('great') || lowerText.includes('amazing') || lowerText.includes('wonderful') || lowerText.includes('incredible')) return '⭐';
    if (lowerText.includes('help') || lowerText.includes('support')) return '🤝';
    if (lowerText.includes('happy') || lowerText.includes('joy')) return '😊';
    if (lowerText.includes('creative') || lowerText.includes('idea')) return '💡';
    if (lowerText.includes('positive') || lowerText.includes('energy')) return '✨';
    if (lowerText.includes('kind') || lowerText.includes('care')) return '💕';
    return '😊'; // default
  };
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2 font-poppins">Start Spreading Smiles</h2>
